...
public class Search extends Module {
	...
	private ChunkProcessor processor = new ChunkProcessor(1,
							...
							if (list.contains(state.getBlock())) {
								...
								if (log) {
									...
									if (be != null) {
										logBuilder
										.append(" BlockEntity")
										.append(be.saveWithFullMetadata(mc.level.registryAccess()).toString());
									}
			...
			(cp, chunk) ->
				foundBlocks.removeIf(pos
						-> pos.getX() >= cp.getMinBlockX()
						&& pos.getX() <= cp.getMaxBlockX()
						&& pos.getZ() >= cp.getMinBlockZ()
	...
	public void onTick(EventTick event) {
		...
		if (!prevBlockList.equals(blockList) || oldViewDistance != mc.options.renderDistance().get()) {
			foundBlocks.clear();

			processor.submitAllLoadedChunks();

			prevBlockList = new HashSet<>(blockList);
			oldViewDistance = mc.options.renderDistance().get();
		}
	}
		...
		for (BlockPos pos : foundBlocks) {
			...
			if (mode == 0 || mode == 2) {
				...
				for (AABB box: voxelShape.toAabbs()) {
					Renderer.drawBoxFill(box.move(pos), QuadColor.single(color[0], color[1], color[2], fillAlpha));
				}
			}
			...
			if (mode == 0 || mode == 1) {
				...
				for (AABB box: voxelShape.toAabbs()) {
					Renderer.drawBoxOutline(box.move(pos), QuadColor.single(color[0], color[1], color[2], 255), outlineWidth);
				}
			}
	...
	public int[] getColorForBlock(BlockState state, BlockPos pos) {
		...
		int color = state.getMapColor(mc.level, pos).col;
