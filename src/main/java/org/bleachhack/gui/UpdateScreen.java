...
public class UpdateScreen extends WindowScreen {
	...
	public void init() {
		...
		addWindow(new Window(width / 2 - wd,
				height / 16,
				width / 2 + wd,
				height - height / 16, String.format("BleachHack Update [%s -> %s]", BleachHack.VERSION, updateJson.get("name").getAsString()), (java.util.function.Supplier<ItemStack>) () -> org.bleachhack.util.SafeItem.of(Items.GLAZED_TERRACOTTA.pick(DyeColor.MAGENTA))));
		...
		addWindow(new Window(width / 2 - wd,
				height / 2 - 15,
				width / 2 + wd,
				height / 2 + 15, "Error updating!", (java.util.function.Supplier<ItemStack>) () -> org.bleachhack.util.SafeItem.of(Items.BANNER.pick(DyeColor.RED)), true));
