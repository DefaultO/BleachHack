...
public class CmdGive extends Command {
	...
	public void onCommand(String alias, String[] args) throws CmdSyntaxException, CommandSyntaxException {
		...
		if (args[0].equalsIgnoreCase("preset")) {
			...
			ItemStack item = new ItemStack(args2.equalsIgnoreCase("egg") ? Items.STRIDER_SPAWN_EGG : args2.equalsIgnoreCase("chest") ? Items.CHEST : Items.DYED_SHULKER_BOX.pink());
			...
			if (args[1].equalsIgnoreCase("negs")) {
				...
				tag = TagParser.parseCompoundFully("{BlockEntityTag:{Items:[{Slot:0b,id:\"minecraft:firework_rocket\",Count:1b,tag:{Fireworks:{Explosions:[{Type:0}" + s1
			...
			if (args2.equalsIgnoreCase("egg")) {
				...
				ct.put("EntityTag", TagParser.parseCompoundFully("{Time:1,id:\"minecraft:falling_block\",BlockState:{Name:\"minecraft:chest\"}}"));
				((CompoundTag) ct.get("EntityTag")).put("TileEntityData", tag.get("BlockEntityTag"));

				setLegacyNbt(item, ct);
			} else {
				setLegacyNbt(item, tag);
			}
		...
		ItemStack item = new ItemStack(
				BuiltInRegistries.ITEM.getValue(Identifier.parse("minecraft:" + args[0].toLowerCase(Locale.ENGLISH))));
		...
		if (args.length >= 3 && NumberUtils.isCreatable(args[2]))
			item.setDamageValue(NumberUtils.createNumber(args[2]).intValue());
		if (args.length >= 4)
			try {
				setLegacyNbt(item, TagParser.parseCompoundFully(args[3]));
	...
	// TODO(26.2): item NBT became data components in modern MC; these tags are still in the
	// pre-1.20.5 format (display.Name, BlockEntityTag with legacy per-item "tag"s), which the
	// game no longer reads off an ItemStack. Stored as CUSTOM_DATA so nothing is lost, but the
	// preset payloads need a rewrite to component SNBT to actually work again.
	private static void setLegacyNbt(ItemStack stack, CompoundTag tag) {
