	...
	public void onCommand(String alias, String[] args) throws CmdSyntaxException, CommandSyntaxException {
		...
		if (args[0].equalsIgnoreCase("preset")) {
			...
			if (args[1].equalsIgnoreCase("negs")) {
				...
				tag = TagParser.parse("{BlockEntityTag:{Items:[{Slot:0b,id:\"minecraft:firework_rocket\",Count:1b,tag:{Fireworks:{Explosions:[{Type:0}" + s1
			...
			if (args2.equalsIgnoreCase("egg")) {
				...
				ct.put("EntityTag", TagParser.parse("{Time:1,id:\"minecraft:falling_block\",BlockState:{Name:\"minecraft:chest\"}}"));
		...
		ItemStack item = new ItemStack(
				BuiltInRegistries.ITEM.get(new Identifier("minecraft:" + args[0].toLowerCase(Locale.ENGLISH))));
		...
		if (args.length >= 4)
			try {
				item.setNbt(TagParser.parse(args[3]));
